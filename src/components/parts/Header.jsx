    ...
    <header className={`fixed top-0 left-0 w-full z-50 backdrop-blur-md border-b transition-colors duration-300 ${
      ...
      <div className="container mx-auto px-5 h-20 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-3">
          ...
          <div>
            <span className={`font-bold tracking-widest block text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>ООО «ИМО»</span>
            <span className="text-[10px] font-mono text-cyan-400 tracking-wider">{t.companySub}</span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-3" ref={menuRef}>
          ...
          <button
            onClick={toggleTheme}
            className={`p-2.5 border rounded-xl transition-all duration-300 flex items-center justify-center shadow-lg cursor-pointer ${
              isDark 
                ? 'bg-zinc-900/80 border-zinc-800 text-yellow-400 hover:border-yellow-400/50 hover:shadow-[0_0_15px_rgba(250,204,21,0.15)]' 
                : 'bg-slate-100 border-slate-200 text-slate-800 hover:border-cyan-500/50 hover:shadow-[0_0_15px_rgba(6,182,212,0.15)]'
            }`}
            title="Сменить тему"
          >
            {isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5 text-cyan-600" />}
          </button>
          ...
          <div className="relative">
            <button
              ...
              className={`p-2.5 border rounded-xl transition-all flex items-center justify-center shadow-lg cursor-pointer ${
                isDark 
                  ? 'bg-zinc-900/80 border-zinc-800 hover:border-cyan-500/50 text-white' 
                  : 'bg-slate-100 border-slate-200 hover:border-cyan-500/50 text-slate-900'
              }`}
            >
